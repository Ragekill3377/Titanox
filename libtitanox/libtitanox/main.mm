...
+ (BOOL)addBreakpointAtAddressNew:(void *)original withHook:(void *)hook {
    if (!original || !hook) {
        NSLog(@"[ERROR] addBreakpointAtAddressNew: invalid params. original=%p, hook=%p", original, hook);
        return NO;
    }
    ...
    if (res) {
        NSLog(@"Added a breakpoint at address: %p", original);
    } else {
        NSLog(@"Failed to add breakpoint at address: %p", original);
    }
    return res;
}
