...
#pragma mark - Breakpoint hook (old, not reccomended. may not work without JIT)
...
+ (void)addBreakpointAtAddress:(void *)target replacement:(void *)replacement outOriginal:(void **)orig {
    [self initBrk];
    THLog(@"Adding a breakpoint...");
    add_breakpoint(target, replacement, orig);
    THLog(@"Added brk hook for %p", target);
}

// NEW breakpoint hook with NO orig.
+ (BOOL)addBreakpointAtAddressNew:(void *)original withHook:(void *)hook {
    if (!original || !hook) {
        THLog(@"[ERROR] addBreakpointAtAddressNew: invalid params. original=%p, hook=%p", original, hook);
        return NO;
    }
...
+ (void)LHHookFunction:(void*)target_function 
    ...
    THLog(@"[LHHookFunction] Hooking into library at path: %@", libPath);

    void *handle = dlopen([libPath UTF8String], RTLD_NOW | RTLD_NOLOAD);
    if (!handle) {
        THLog(@"[LHHookFunction] Failed to open library: %s", dlerror());
        return;
    }

    THLog(@"[LHHookFunction] Hooking target function at %p with hook function at %p", target_function, hook_function);
    LHHookFunction(target_function, hook_function, out_hook_ref);

    dlclose(handle);
    THLog(@"[LHHookFunction] Hooking complete and library closed");
}
...
+ (void *)ReadMemAtAddr:(unsigned long long)address size:(size_t)size {
    if (size == 0 || address == 0) {
        THLog(@"Error: Invalid address or size.");
        return NULL;
    }
    ...
    if (buffer == NULL) {
        THLog(@"Error: Failed to alloc mem for buffer.");
        return NULL;
    }
    ...
    if (kr != KERN_SUCCESS || bytesRead != size) {
        THLog(@"Error: vm_read_overwrite failed for address: 0x%llx, size: %zu. Error code: %d", address, size, kr);
        free(buffer);
        return NULL;
    }
    ...
    if (res == NULL) {
        THLog(@"Error: Failed to allocate memory for buffer.");
        free(buffer);
        return NULL;
    }
...
+ (BOOL)JRwriteMemory:(uint64_t)address withData:(const void *)data length:(size_t)length {
    if (JRengine == nullptr) {
        THLog(@"JRMemoryEngine is not initialized. Attempting to initialize...");
        [self initJRMemEngine];
    }

    if (JRengine == nullptr) {
        THLog(@"Error: JRMemoryEngine failed to initialize.");
        return NO;
    }

    if (address == 0 || data == nullptr || length == 0) {
        THLog(@"Error: Invalid address (0x%llx), data, or length.", address);
        return NO;
    }
    ...
    THLog(@"Successfully wrote %zu bytes to address 0x%llx.", length, address);
    return YES;
}
...
+ (void)CGPWriteMemoryAtAddress:(void *)address 
    ...
    if (CGPmemoryEngine) {
        ...
        if (!address || !data) {
            THLog(@"Address and/or data is invalid.");
            return;
        }
