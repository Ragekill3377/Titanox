...
kern_return_t TotallyNotVM::read(mach_vm_address_t address, void *buffer, mach_vm_size_t size) {
    ...
    if (!(reply.header.msgh_bits & MACH_MSGH_BITS_COMPLEX)) {
        struct { mach_msg_header_t h; NDR_record_t ndr; kern_return_t ret; } *err =
            (decltype(err))&reply;
        THLog(@"read failed with ret_code: %d", err->ret);
        return err->ret;
    }
    ...
    if (!reply.data_desc.address) {
        THLog(@"read failed: reply data address null");
        return KERN_FAILURE;
    }

    memcpy(buffer, reply.data_desc.address, reply.dataCnt);
    vm_deallocate(mach_task_self(), (vm_address_t)reply.data_desc.address, reply.dataCnt);
