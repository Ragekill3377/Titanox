...
bool LHUnhookFunction(LHHookRef* hook_ref) {
    ...
    if (!THPatchMem::PatchMemory(hook_ref->original_function, hook_ref->trampoline, 12)) {
        return false;
    }
#elif x86_64 == 1
    ...
    if (!THPatchMem::PatchMemory(hook_ref->original_function, hook_ref->trampoline, 16)) {
        return false;
    }
#endif
