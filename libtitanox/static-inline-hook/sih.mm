...
HookBlock* MachOHooker::find_hook_block(void* base, uint64_t vaddr) const {
    ...
    for (uint32_t i = 0; i < header->ncmds; ++i) {
        if (lc->cmd == LC_SEGMENT_64) {
            ...
            if (seg->fileoff == 0 && seg->filesize > 0 && text_base_vaddr == 0) {
                text_base_vaddr = seg->vmaddr;
            }
        }
        lc = (load_command*)((uint64_t)lc + lc->cmdsize);
    }
...
bool MachOHooker::apply_inline_patch(HookBlock* block, uint64_t func_rva, void* func_data, uint64_t target_rva, void* target_data, const std::string& patch_bytes)
{
    ...
    block->patched_vaddr  = target_rva;            
    block->code_vaddr     = target_rva;
    block->code_size      = trampoline_total;
...
void* MachOHooker::hook_function(uint64_t vaddr, void* replacement) {
    ...
    uint64_t text_base_vaddr = 0;
    auto* hdr = (mach_header_64*)base;
    auto* lc = (load_command*)((uint64_t)hdr + sizeof(*hdr));
    ...
    intptr_t slide = (intptr_t)base - (intptr_t)text_base_vaddr;
    uint8_t* trampoline = (uint8_t*)(block->patched_vaddr + slide);
    uint64_t return_addr = block->original_vaddr + slide;
    memcpy(trampoline + 20, &return_addr, sizeof(return_addr));
    uint8_t* hook_site = (uint8_t*)(block->hook_vaddr + slide);
    uint64_t replacement_addr = (uint64_t)replacement;
    memcpy(hook_site + 8, &replacement_addr, sizeof(replacement_addr));
