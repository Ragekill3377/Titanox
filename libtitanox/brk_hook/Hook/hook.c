...
#pragma clang diagnostic pop
    ...
    if (count > breakpoints) return false; // max are the hw breakpoints available. this cannot be exceeded.
...
bool unhook(void *old[], int count) {
    arm_debug_state64_t state = {};
    printf("[HOOK] Removing breakpoints...\n");
    
    for (int i = 0; i < count; i++) {
    printf("[HOOK] Removing breakpoint at %p\n", old[i]);
    state.__bvr[i] = 0;
    state.__bcr[i] = 0;
